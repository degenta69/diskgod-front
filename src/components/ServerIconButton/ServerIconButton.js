...
const ServerIconButton = ({ show, usersNumber, profile, data }) => {
  ...
  return (
    <>
      <BlackTooltip
        ...
        arrow
        title={data?.chatName ? data?.chatName : "Home"}
      >
        <div
          className={`rounded-3xl servericon cursor-pointer ${
            profile ? "hover:bg-blue-600" : ""
          ...
          {show ? (
            <div className="hole h-6 w-7 bg-sidebarBG rounded-full absolute ">
              <div className="badge h-4 w-5 bg-badgeBG rounded-full absolute ">
                <p className="badgeFont">{usersNumber ? usersNumber : 54}</p>
              </div>
            </div>
          ) : (
            ""
